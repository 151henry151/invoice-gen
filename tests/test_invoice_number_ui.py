...
def test_create_invoice_page_assigns_readonly_number(auth_client, app, test_user):
    with app.app_context():
        db.session.query(InvoiceDraft).filter_by(user_id=test_user.id).delete()
        db.session.query(Invoice).filter_by(user_id=test_user.id).delete()
        setting = (
            db.session.query(Setting)
            .filter_by(user_id=test_user.id, key="next_invoice_number")
            .first()
        )
        if setting:
            setting.value = "2040"
    ...
    resp = auth_client.get("/create_invoice?new=1", follow_redirects=True)
